...
extern "C" {
#endif
SWIGINTERN int Swig_var_My_pi_set(PyObject *_val) {
  {
    double val;
    int res = SWIG_AsVal_double(_val, &val);
    if (!SWIG_IsOK(res)) {
      SWIG_exception_fail(SWIG_ArgError(res), "in variable '""My_pi""' of type '""double""'");
    }
    My_pi = (double)(val);
  }
  return 0;
...
SWIGINTERN PyObject *Swig_var_My_pi_get(void) {
  PyObject *pyobj = 0;
  
  pyobj = SWIG_From_double((double)(My_pi));
  return pyobj;
}
...
# endif
  ...
  Py_DECREF(globals);
  SWIG_addvarlink(globals, "My_pi", Swig_var_My_pi_get, Swig_var_My_pi_set);
#if PY_VERSION_HEX >= 0x03000000
  return m;
#else
  return;
